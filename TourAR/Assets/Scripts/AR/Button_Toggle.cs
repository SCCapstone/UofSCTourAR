...
public class Button_Toggle : MonoBehaviour
{
    public enum ARUItype
    {
        ...
        Settings,
        Back
    } //determine which type of button this is being applied to

    [Tooltip("Determine the type of button script will be used on.")]
    [SerializeField]
    public ARUItype objtype;
    ...
    */
    ...
    IEnumerator ARButton()
    {
        ...
        {
            Debug
                .Log("Toggle: Box is already open - " +
                boxTopAnimator.GetBool("isOpen"));
        }
    ...
    {
        //Prints the bool state true/false and current state name
    ...
    private void PrintStateName(Animator anim, string name)
    {
        ...
        {
            //find the matching name of the state the animator is in and print
            if (anim.GetCurrentAnimatorStateInfo(0).IsName(state))
            {
                Debug
                    .Log("Toggle_ The current " +
                    name +
                    " anim state is: " +
                    state);
            }
